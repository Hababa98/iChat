...
class EcommerceOrderList extends Controller
{
  ...
  {
    $pendingOrder = Order::where('order_status', 1)->count();
    $completedOrder = Order::where('order_status', 2)->count();
    $cancleOrder = Order::where('order_status', 0)->count();
  ...
  public function getOrderListData(Request $request)
  {
    ...
    $records = Order::select('users.first_name', 'users.last_name', 'users.email', 'users.mobile', 'users.image', 'orders.user_id', 'orders.id', 'orders.order_status', 'orders.order_id', 'orders.created_at', 'orders.payment_mode')->orderBy($columnName, $columnSortOrder)
  ...
  public function getPendingOrderList(Request $request)
  {
    ...
    $columnSortOrder = $order_arr[0]['dir'];
    $searchValue = $search_arr['value'];
    $totalRecords = Order::select('count(*) as allcount')->where('order_status', 1)->count();
    $totalRecordswithFilter = Order::select('count(*) as allcount')->where('order_id', 'like', '%' . $searchValue . '%')->where('order_status', 1)->count();
    // dd($columnName, $columnSortOrder);
    $records = Order::select('users.first_name', 'users.last_name', 'users.email', 'users.mobile', 'users.image', 'orders.user_id', 'orders.id', 'orders.order_status', 'orders.order_id', 'orders.created_at', 'orders.payment_mode')
