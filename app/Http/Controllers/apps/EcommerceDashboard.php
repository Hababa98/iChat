...

{
  ...
  {
    try {
      // Perform some operations with Firebase
    ...
    // return view('content.apps.app-ecommerce-dashboard', compact('totalSales', 'totalProduct', 'totalUser', 'totalOrders'));
  }
...

  {
    ...
    $records = Order::select('users.id', 'users.first_name', 'users.last_name', 'users.email', 'users.mobile', 'users.image', 'orders.user_id', 'orders.id as orders_id', 'orders.order_status', 'orders.order_id', 'orders.created_at')->orderBy($columnName, $columnSortOrder)
